...
for FN=1:filenumber
    [filecontent,map] = imread(filelist_aftercheck{FN});
    [psize_x,psize_y,psize_z]=size(filecontent);

    pix_num(FN)=0;
    for i=1:psize_y
        for j=1:psize_x
            pixel(i,j) = filecontent(i,j,1);
            if(pixel(i,j) > Pthreshold)
                pix_num(FN)=pix_num(FN)+1;
                rou(i,j)=double(4.46556)*double(pixel(i,j))-double(121.23378);%mg/cm3
